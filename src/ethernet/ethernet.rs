...
use super::timer::CountDownTimer as Timer;
// use crate::pins;
...
static mut ETHERNET_MUTEX: Option<Interface> = None;
...
pub struct Interface<'a> {
    ...
    interface: Option<EthernetInterface<'a, ethernet::EthernetDMA<'a, 4, 4>>>,
...
impl<'a> Interface<'a> {
    ...
    pub fn start(
        ...
        let timeout_timer =
            match interface.up(mac_address, ip_address, eth1mac, ccdr_clocks, timeout_timer) {
    ...
    /// not interrupt free anymore lol
        ...
        F: FnOnce(&mut Interface<'static>) -> R,
    {
        ...
        //         ETHERNET_MUTEX.borrow(cs).borrow_mut().as_mut()
        //     {
        ...
        // cortex_m::interrupt::free(|cs| {
        if let Some(ethernet_interface) = unsafe { &mut ETHERNET_MUTEX } {
            f(ethernet_interface)
        } else {
            panic!("Ethernet interface has not been started");
        }
    ...
    fn up(
        ...
        unsafe {
            ETHERNET_STORAGE.ip_addrs =
                [IpCidr::new(Ipv4Address::from_bytes(ip_address).into(), 0)];
        }

        let neighbor_cache =
            NeighborCache::new(unsafe { &mut ETHERNET_STORAGE.neighbor_cache_storage[..] });
    ...
    pub fn poll(&mut self) -> Result<bool, smoltcp::Error> {
        ...
        self.interface
            .as_mut()
            .unwrap()
            .poll(&mut self.sockets.as_mut().unwrap(), timestamp)
    }
    ...
    pub fn poll_delay(&mut self) -> Option<Duration> {
        ...
        self.interface
            .as_mut()
            .unwrap()
            .poll_delay(&mut self.sockets.as_mut().unwrap(), timestamp)
    }
